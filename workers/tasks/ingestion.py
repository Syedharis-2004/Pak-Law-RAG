...
def process_document_task(self, document_id: str, job_id: str) -> str:
    ...
    try:
        ...
        logger.error(f"Ingestion failed for Document ID: {document_id}. Error: {str(e)}")
...
def delete_document_vector_task(document_id: str) -> str:
    ...
    collections = ["acts", "ordinances", "rules", "judgments", "contracts", "legal_documents"]
    ...
    for collection in collections:
        try:
            ...
            logger.warn(f"Failed deleting points from collection '{collection}': {str(e)}")
